...
std::unique_ptr<InterferenceAnalysis> InterferenceAnalysisFactory::create(const Problem& problem, const std::string& analysis_type) {
    if (analysis_type == "semantic") {
        std::cout << "Creating SemanticInterferenceAnalysis (lazy semantic)" << std::endl;
        return std::make_unique<SemanticInterferenceAnalysis>(problem);
    } else if (analysis_type == "eager-semantic") {
        std::cout << "Creating EagerSemanticInterferenceAnalysis (eager semantic)" << std::endl;
        return std::make_unique<EagerSemanticInterferenceAnalysis>(problem);
    } else if (analysis_type == "lazy") {
        std::cout << "Creating LazyInterferenceAnalysis (lazy syntactic)" << std::endl;
        return std::make_unique<LazyInterferenceAnalysis>(problem);
    } else if (analysis_type == "eager") {
        std::cout << "Creating EagerInterferenceAnalysis (eager syntactic)" << std::endl;
        return std::make_unique<EagerInterferenceAnalysis>(problem);
    } else {
        throw std::invalid_argument("Invalid analysis_type '" + analysis_type + "'. Valid options: 'eager', 'eager-semantic', 'lazy', 'semantic'");
    }
}
