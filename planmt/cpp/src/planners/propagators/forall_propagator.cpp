...
ForallPropagator::ForallPropagator(z3::solver& solver, const Problem& problem)
    ...
    register_fixed(); // Enables fixed() callback
    register_final(); // Enables final() callback
...
void ForallPropagator::final() {
    ...
    //std::cout << "ForallPropagator::final() called - model found" << std::endl;
}
...
void ForallPropagator::register_timestep_variables(int timestep) {
    if (!encoder_) {
        throw std::runtime_error("ForallPropagator not initialized before registering variables");
    }
...
void ForallPropagator::handle_state_variable_fixed(
    const z3::expr& var, 
    const z3::expr& value,
    const Fluent& fluent, 
    int timestep) {
...
void ForallPropagator::handle_action_variable_fixed(
    const z3::expr& var, 
    const z3::expr& value,
    const Action& action, 
    int timestep) {
