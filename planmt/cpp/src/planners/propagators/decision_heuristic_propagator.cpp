...
DecisionHeuristicPropagator::DecisionHeuristicPropagator(z3::solver& solver, const Problem& problem, const BaseEncoder& encoder)
    : z3::user_propagator_base(&solver), encoder_(dynamic_cast<const ReifiedGroundedEncoder*>(&encoder)), 
      problem_(&problem), ctx_(&solver.ctx()), solver_(&solver), goal_timestep_(-1) {
