...
ForallOnDemandPropagator::ForallOnDemandPropagator(z3::solver& solver, const Problem& problem)
...
void ForallOnDemandPropagator::fixed(z3::expr const &ast, z3::expr const &value) {
    if (!value.is_true()) {
...
void ForallOnDemandPropagator::perform_forall_propagation(const Action& action, int timestep, const z3::expr& action_var) {
    ...
    for (const Action& action_to_negate : interfering_actions) {
...
z3::user_propagator_base* ForallOnDemandPropagator::fresh(z3::context& ctx) {
    // For now, return null to indicate we don't support fresh instances
    // TODO: Implement proper fresh instance creation if needed
    return nullptr;
}

void ForallOnDemandPropagator::initialize(z3::solver& solver, const GroundedEncoder& encoder) {
...
void ForallOnDemandPropagator::register_timestep_variables(int timestep) {
...
PropagatorType ForallOnDemandPropagator::get_type() const {
    return PropagatorType::FORALL_ON_DEMAND;
}

void ForallOnDemandPropagator::build_reverse_interference_lookup() {
